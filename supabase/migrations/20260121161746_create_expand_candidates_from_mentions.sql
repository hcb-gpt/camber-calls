-- Migration: create_expand_candidates_from_mentions
-- Purpose: Scan transcript for contact names mentioned (not just speakers)
--          and boost projects where those contacts have concentrated affinity
...
BEGIN
  ...
  ORDER BY cap.max_weight DESC;
END;
$$;

COMMENT ON FUNCTION expand_candidates_from_mentions IS 
'Scans transcript for contact names (not speakers) who have concentrated project affinity.
Solves the floater correspondent problem.';;
