-- Stage A launch metrics: correction-rate telemetry views (analytics-only).
-- This migration is non-invasive: creates/updates reporting views only.

-- Daily correction metric from review resolutions + override_log truth table.
...
-- Daily correction metric broken down by model/prompt (latest attribution per span).
...
comment on view public.v_launch_review_correction_by_model_daily is
  'Daily correction telemetry by model_id/prompt_version. Uses latest attribution per span for reporting breakdown.';
