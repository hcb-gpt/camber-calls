...
-- Fix promotion function to use claim_project_id (per-claim attribution) instead of project_id (legacy per-call)
-- Also adds source_run_id for rollback support
...
BEGIN
  ...
  LOOP
    -- GATE: Skip claims with NULL claim_project_id (ambiguous attribution)
    ...
    ELSE
      -- Promote to belief_claims using claim_project_id (not legacy project_id)
      ...
      INSERT INTO belief_claims (
        ...
        created_at,
        updated_at,
        source_run_id  -- NEW: for rollback support
      ) VALUES (
        v_new_claim_id,
        v_claim.claim_project_id,  -- FIXED: use claim_project_id not project_id
        ...
        END,
        -- Use attribution_confidence if available, else derive from warrant_level
        COALESCE(
        ...
        NOW(),
        NOW(),
        p_run_id  -- NEW: source_run_id for rollback
      );
...
COMMENT ON FUNCTION promote_journal_claims_to_belief(uuid) IS 
  'v2: Uses claim_project_id for per-claim attribution, skips NULL projects, sets source_run_id for rollback';
;
