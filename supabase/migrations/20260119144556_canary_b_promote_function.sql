...
-- Canary-B: Create promotion function
CREATE OR REPLACE FUNCTION promote_journal_claims_to_belief(p_run_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  LOOP
    ...
    ELSE
      ...
      INSERT INTO belief_claims (
        ...
        END,
        -- Confidence based on warrant_level
        CASE v_claim.warrant_level
          WHEN 'high' THEN 0.85
  ...
  RETURN jsonb_build_object(
    ...
    'review_reasons', jsonb_build_object('missing_pointer_time', v_claims_routed)
  );
END;
$$;

COMMENT ON FUNCTION promote_journal_claims_to_belief(UUID) IS 'Promotes claims from J0 staging (journal_claims) to J1 ledger (belief_claims). Routes claims with NULL timestamps to review.';
;
