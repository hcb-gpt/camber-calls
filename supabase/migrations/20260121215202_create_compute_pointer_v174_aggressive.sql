...
-- v1.7.4 Aggressive pointer computation - strips speaker labels from transcript
...
BEGIN
  ...
  -- Clean search text: strip speaker label prefix
  ...
  -- Strategy 5: Strip speaker labels from transcript entirely
  IF v_found_pos = 0 THEN
    -- Remove "Speaker Name: " patterns  
    v_clean_transcript := REGEXP_REPLACE(LOWER(v_transcript), E'^[A-Za-z ]+:\\s*', '', 'gm');
    v_clean_transcript := REGEXP_REPLACE(v_clean_transcript, E'\\s+', ' ', 'g');
    v_clean_search := REGEXP_REPLACE(LOWER(v_search_text), E'^[A-Za-z ]+:\\s*', '', 'g');
    v_clean_search := REGEXP_REPLACE(v_clean_search, E'\\s+', ' ', 'g');
    v_found_pos := POSITION(LEFT(v_clean_search, 25) IN v_clean_transcript);
  END IF;
