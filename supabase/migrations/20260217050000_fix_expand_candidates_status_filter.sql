-- Fix expand_candidates_from_mentions to use canonical status filter
-- Was: ('active', 'warranty', 'pre-construction')
-- Now: ('active', 'warranty', 'estimating')
-- Aligns with all other 16 locations in the codebase
...
'v5: Scans transcript for contact names with concentrated project affinity.
Uses word-boundary regex (\m...\M) for matching.
Fixed: Status filter aligned to canonical set (active, warranty, estimating).
