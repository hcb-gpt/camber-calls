...
BEGIN
  ...
  SELECT DISTINCT ON (ac.project_id, ac.alias)
    ...
    END as match_type,
    CASE
      WHEN v_transcript_lower ~ ('\y' || LOWER(ac.alias) || '\y') THEN 1.0
      WHEN v_transcript_lower LIKE '%' || LOWER(ac.alias) || '%' THEN 0.9
      ELSE 0.0
    END as score
...
Returns exact matches only - removed phonetic matching to avoid false positives.';
;
