...
Deno.serve(async (req: Request) => {
  ...
  try {
    // IDEMPOTENCY
    if (!id_gen) {
      const { error } = await db.from('idempotency_keys').insert({ key: iid, interaction_id: iid, source: raw.source || 'edge', router_version: 'v3.9.0' });
      if (error && (error.message?.includes('duplicate') || error.message?.includes('23505'))) {
        await db.from('event_audit').insert({ interaction_id: iid, gate_status: 'SKIP', gate_reasons: ['G1_DUPLICATE_EXACT'], source_system: 'edge_v3.9', source_run_id: run_id, pipeline_version: 'v3.9' });
    ...
    // AUDIT STARTED
    const { data: ad } = await db.from('event_audit').insert({ interaction_id: iid, gate_status: 'STARTED', gate_reasons: [], source_system: 'edge_v3.9', source_run_id: run_id, pipeline_version: 'v3.9', processed_by: 'process-call', persisted_to_calls_raw: false, i1_phone_present: !!(raw.from_phone || raw.to_phone), i2_unique_id: !id_gen }).select('id').single();
    ...
    if (n.transcript) {
      ...
      const { data: aliasRows } = await db.from('v_project_alias_lookup').select('project_id, alias');
    ...
    for (const [pid, meta] of candidatesById) {
      const isExistingProject = meta.sources.includes('interactions_existing_project');
      const hasMentionedContact = meta.sources.includes('mentioned_contact_affinity');
    ...
    rankedCandidates.sort((a, b) => {
      if (a.assigned !== b.assigned) return (b.assigned ? 1 : 0) - (a.assigned ? 1 : 0);
      if (b.alias_matches.length !== a.alias_matches.length) return b.alias_matches.length - a.alias_matches.length;
      if (b.affinity_weight !== a.affinity_weight) return b.affinity_weight - a.affinity_weight;
    ...
    if (rankedCandidates.length > 0) {
      ...
      if (winner.sources.length >= 3) project_confidence = Math.min(project_confidence + 0.1, 0.99);
      if (winner.alias_matches.length >= 2) project_confidence = Math.min(project_confidence + 0.05, 0.99);
    }
    ...
    // AUDIT FINAL
    if (audit_id) await db.from('event_audit').update({
