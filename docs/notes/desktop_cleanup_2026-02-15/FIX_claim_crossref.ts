...
const STOPWORDS = new Set([
  ...
  "he", "she", "they", "them", "his", "her", "we", "our", "you", "your",
  "i", "me", "my", "mine", "who", "what", "which", "whom",
  // Prepositions / conjunctions
  "in", "on", "at", "to", "for", "of", "with", "from", "by", "as",
  "into", "about", "up", "out", "over", "after", "before", "between",
  "and", "or", "but", "so", "if", "then", "than", "because", "when",
  "where", "how", "while", "until", "although", "though",
  // Common verbs (non-construction)
  "is", "are", "was", "were", "be", "been", "being", "have", "has", "had",
  "do", "does", "did", "will", "would", "could", "should", "shall", "may",
  "might", "can", "must", "get", "got", "getting", "let", "make", "made",
  "take", "took", "give", "gave", "go", "going", "went", "gone", "come",
  "came", "say", "said", "says", "tell", "told", "know", "knew", "think",
  "thought", "see", "saw", "look", "want", "wanted", "need", "needed",
  "just", "like", "well", "here", "there", "now", "also", "still",
  // Filler / conversational
  "yeah", "yes", "no", "not", "okay", "ok", "right", "um", "uh", "hmm",
  "gonna", "gotta", "wanna", "kinda", "sorta", "actually", "basically",
  "really", "very", "much", "some", "any", "all", "each", "every",
  "other", "another", "more", "most", "enough", "too", "already",
  ...
  // Misc
  "way", "lot", "back", "even", "through", "down", "only", "been",
  "same", "own", "part", "first", "last", "next", "new", "old",
  "big", "little", "long", "good", "bad", "great", "sure", "probably",
]);
...
const CONSTRUCTION_TERMS = new Set([
  ...
  "marble", "granite", "quartz", "quartzite", "soapstone", "travertine",
  "limestone", "onyx", "porcelain", "ceramic", "slate", "sandstone",
  "countertop", "countertops", "backsplash", "slab", "slabs",
  // Materials - Wood
  "hardwood", "softwood", "plywood", "shiplap", "wainscoting",
  "trim", "molding", "moulding", "baseboard", "crown", "casing",
  "mantel", "mantle", "stair", "stairs", "staircase", "railing",
  "banister", "newel", "baluster",
  // Materials - Flooring
  "tile", "tiles", "tiling", "grout", "mortar", "thinset",
  "flooring", "hardwoods", "laminate", "vinyl", "carpet", "rug",
  "subfloor", "underlayment",
  // Plumbing
  "tub", "bathtub", "shower", "faucet", "toilet", "sink", "vanity",
  "plumbing", "pipe", "pipes", "drain", "drainage", "sewer",
  "tankless", "valve", "fixture", "fixtures",
  // Electrical
  "electrical", "wiring", "panel", "breaker", "outlet", "switch",
  "lighting", "light", "lights", "chandelier", "sconce", "recessed",
  "dimmer", "generator", "transformer",
  // HVAC
  "hvac", "furnace", "ductwork", "ducts", "thermostat", "compressor",
  "condenser",
  // Structural
  "foundation", "footing", "footer", "beam", "joist", "rafter",
  "truss", "header", "column", "post", "pier", "slab",
  "framing", "studs", "sheathing",
  // Exterior
  "siding", "roofing", "shingle", "shingles", "gutter", "gutters",
  "soffit", "fascia", "flashing", "dormer", "skylight",
  "window", "windows", "door", "doors", "garage",
  // Finishes
  "paint", "primer", "stain", "varnish", "lacquer", "polyurethane",
  "drywall", "plaster", "texture", "wallpaper", "caulk", "caulking",
  // Appliances / Fixtures
  "appliance", "appliances", "range", "oven", "refrigerator", "dishwasher",
  "microwave", "hood", "vent",
  // Trade roles
  "plumber", "electrician", "painter", "framer", "drywaller",
  "roofer", "mason", "tiler", "carpenter", "cabinetmaker",
  "contractor", "subcontractor", "inspector",
  // Construction phases
  "demolition", "demo", "excavation",
  "inspection", "walkthrough",
  "closeout", "warranty", "permit",
  // Cabinetry
  "cabinet", "cabinets", "cabinetry", "drawer", "drawers",
  "shelf", "shelves", "shelving", "pantry", "closet",
]);
...
function extractCompoundTerms(text: string): string[] {
  ...
  const materialColorRegex = /\b([a-z]+)\s+(white|black|gray|grey|gold|blue|green|red|brown|beige|cream|silver|bronze)\b/g;
  ...
  const qtyRegex = /\b(one|two|three|four|five|six|seven|eight|nine|ten|\d+)\s+(slab|tub|panel|sheet|piece|tile|board|cabinet|door|window|fixture)s?\b/g;
    ...
    for (const { term, specificity } of transcriptTerms) {
      ...
      if (matched) {
        ...
        for (let ci = 0; ci < claimList.length; ci++) {
          ...
          const inClaim = term.includes(" ")
            ? ct.includes(term)
            : ctTokens.includes(term) || ctTokens.some(
...
cost us time and probably another $800 for restocking`;
  ...
  const journal_claims: JournalClaimInput[] = [
    // Skelton claims -- matches marble selection + tub topic
    {
      ...
      claim_text: "They picked out some marble -- mystery white from Arizona Tile. Customer is excited about the selection.",
      claim_type: "material_selection",
    },
    {
      project_id: "proj_skelton",
      claim_text: "Tub delivery arrived but size doesn't match the spec. Need to return and reorder correct dimensions.",
      claim_type: "issue",
    },
    {
      project_id: "proj_skelton",
  ...
  for (const r of results) {
    ...
    for (const mc of r.matching_claims) {
      console.log(`    claim: "${mc.claim_text.slice(0, 80)}..." [${mc.claim_type}] overlaps=[${mc.overlap_terms.join(", ")}]`);
    }
    console.log();
  }
  ...
  console.log(`  Max-Min spread <= 0.15: ${(maxGeneric - minGeneric) <= 0.15 ? "PASS" : "FAIL"} (spread=${(maxGeneric - minGeneric).toFixed(3)})`);
...
const _isMainModule = typeof Deno !== "undefined"
  ? (Deno as any).mainModule === `file://${(Deno as any).cwd?.() || ""}/${new URL(import.meta.url).pathname.split("/").pop()}`
