...
-- ================================================
-- PERSIST_CALL_EVENT: Single entry point for call ingestion
-- Enforces invariants, logs to event_audit, conditionally persists
-- ================================================
...
BEGIN
  ...
  IF NOT v_i2_unique THEN
    -- Duplicate = fail (already exists)
    v_gate_status := 'fail';
    v_triage_action := 'reject';
  ELSIF NOT v_i1_phone AND NOT v_i5_lineage THEN
    -- Missing phone AND lineage = needs human
    v_gate_status := 'needs_human';
    v_triage_action := 'queue_human';
  ELSIF NOT v_i1_phone THEN
    -- Missing phone but has lineage = might be fixable
    v_gate_status := 'needs_human';
    v_triage_action := 'queue_human';
  ELSIF NOT v_i5_lineage THEN
    -- Has phone but missing lineage = pass with warning (legacy behavior)
    v_gate_status := 'pass';
...
COMMENT ON FUNCTION persist_call_event(jsonb) IS 'AI-forward gate: validates payload against invariants, logs to event_audit, conditionally persists to calls_raw';
;
