...
-- Migration: create_backfill_scheduler_items_function_v2
-- Purpose: Idempotent unpacking of ai_scheduler_json into scheduler_items (fixed version)

CREATE OR REPLACE FUNCTION backfill_scheduler_items(p_dry_run BOOLEAN DEFAULT TRUE)
RETURNS TABLE (
  out_interaction_id TEXT,
  out_items_found INT,
  out_items_inserted INT,
  out_items_skipped INT
) AS $$
DECLARE
...
BEGIN
  ...
  LOOP
    ...
    IF v_interaction.ai_scheduler_json ? 'tasks' 
      ...
      LOOP
        ...
        IF NOT p_dry_run THEN
          ...
          END IF;
        ELSE
          IF EXISTS (SELECT 1 FROM scheduler_items si WHERE si.interaction_id = v_interaction.uuid_id AND si.item_hash = v_item_hash) THEN
    ...
    IF v_found > 0 THEN
      out_interaction_id := v_interaction.text_id;
      out_items_found := v_found;
      out_items_inserted := v_inserted;
      out_items_skipped := v_skipped;
      RETURN NEXT;
    END IF;
  END LOOP;
END;
