...
```bash
# 1. Create credential store (fallback)
mkdir -p ~/.camber

# 2. Copy credentials from source (fallback file)
...
# 3. Import into Keychain (preferred)
~/.camber/keychain-import.sh

# 4. Install auto-loader in shell profile
cat >> ~/.zshrc << 'EOF'
...
# 5. Copy loader script to ~/.camber/
...
**If you must use the fallback file directly:**
```bash
...
**Primary credential store:** macOS Keychain service `camber`  
**Fallback file:** `~/.camber/credentials.env` (chmod 600)
