...
1) Fill **Metadata** (runner commit/version, pipeline/deploy refs, timestamps).
2) Attach **Inputs** (manifest path + counts; note any rerun selection rules).
3) Paste **Headline metrics** + **Bucket metrics** (tag-stratified, same buckets each run).
4) Paste **Review contamination** stats (window = `run_started_at_utc` preferred) + poisoning invariant check.
5) List **Top failures** + **Next actions**, and link the artifact directory.
...
- `spans_with_span_id`:
- Evidence packs referenced (optional):
  - `/Users/chadbarlow/Desktop/anchor_catalog_v1_20260215.csv`
  - `/Users/chadbarlow/Desktop/homeowner_override_proofset_20260215.csv`
...
- `staff_leak_rate` = predicted_project in {Sittler…} / total (target: 0 after P0)
...
- `latest_attr(span_id)` = most recent row in `public.span_attributions` for that `span_id`, ordered by `attributed_at DESC` (tie-breaker: `id DESC`).
- A span is **reviewish** if:
  - `latest_attr.decision IN ('review','none')` **OR**
  - `latest_attr.applied_project_id IS NULL`

Rationale: `applied_project_id` is the only durable “this was applied” indicator; reviewish outputs should not produce reusable evidence writes.

### 5.2 Tier J0 (staging contamination) — primary metric
...
- `review_span_count` := count of spans whose latest attribution is reviewish **and** `latest_attr.attributed_at >= since`
...
- Window for contamination: `since = run_started_at_utc` (preferred) or last 7d.
- `review_span_count`:
...
Minimal changes for a GT batch runner:
- Record `run_started_at_utc` and include it in run artifacts.
- After the run, execute the batch contamination query (C) restricted to batch spans, plus a `journal_claims.created_at >= run_started_at_utc` filter.
- Emit in summary:
  - `review_contamination_count`, `contamination_rate`, and poisoning invariant `confirmed_claim_rows_on_reviewish`.

Suggested policy (until a write-gate ships):
- **FAIL** only if poisoning invariant breaks (`confirmed_claim_rows_on_reviewish > 0`).
- Track J0/J1 contamination as trend metrics; do not fail runs on non-zero J0 alone.
